...
if __name__ == '__main__':
    ...
    print(f'Crop mean distance [mm]: {np.mean(models.distance(y_val_crop, y_pred))}')
    for idx in range(8):
        ...
        if np.any(sample_pred) and np.any(sample_true):
            title = f'True: {np.argmax(sample_true)}, pred: {np.argmax(sample_pred)}, \
                        confidence: {np.amax(sample_pred)}'
        ...
        plt.title(title)
        plt.savefig(f'./output/tmp/{idx}drop.png')
        plt.close() 

    # compare (full-size)
    y_pred = models.predict_whole(model, x_val)
    y_true = np.array([np.argmax(y) for y in y_val])
    print(f'Whole images mean distance [mm]: {utils.distance(y_true, y_pred)}')
    ...
    for idx in range(8):
        ...
        plt.imshow(sample_img, cmap='gray')
        plt.title(title)
        plt.savefig(f'./output/tmp/{idx}full.png')
        plt.close() 
